...
*/
    ...
    enableRheoTool_(lookupOrDefault<Switch>("enableRheoTool",false))
/*
*/
{
    calcAlphas();
    alphas_.write();
    nu_.write();  // aded by CDD
}
...
Foam::multiphaseMixture::divTauMF
(
  volVectorField& U
)
{

    tmp<fvVectorMatrix> tdivTauMF = fvm::laplacian( rho()*nu(), U, "laplacian(rho*nu,U)") 
                                  + fvc::div((rho()*nu())*dev2(T(fvc::grad(U))),"div(rho*nu*dev2(T(gradU)))");
    ...
    tmp<volSymmTensorField> ttauMF = iter()*iter().tau();
    ...
    return tdivTauMF + fvc::div(tauMF_,"div(Sum(tau))");
}
*/
...
)
{
    ...
    if (iter().isGNF()) {
    Info<< "***GNF fluid for phase "<< count <<endl; 
    //tmp<fvVectorMatrix> tdivTauMF = fvm::laplacian( iter()*iter().rho()*iter().myNu(), U, "laplacian(rho*nu,U)")
    tdivTauMF = fvm::laplacian( iter()*iter().rho()*iter().myNu(), U, "laplacian(rho*nu,U)")
    + fvc::div( (iter()*iter().rho()*iter().myNu())*dev2(T(fvc::grad(U))),"div(rho*nu*dev2(T(gradU)))");
    } else {
    Info<< "***non-GNF fluid for phase "<< count <<endl; 
//    tdivTauMF = fvm::laplacian( iter()*iter().rho()*iter().etaS(), U, "laplacian(rho*nu,U)")
//    + fvc::div( (iter()*iter().rho()*iter().etaS())*dev2(T(fvc::grad(U))),"div(rho*nu*dev2(T(gradU)))");
    ...
    for (++iter; iter != phases_.end(); ++iter)
    {
     ...
     if (iter().isGNF()){
      Info<< "***GNF fluid for phase "<< count <<endl;         
        ...
        divTauMF += fvm::laplacian( iter()*iter().rho()*iter().myNu(), U, "laplacian(rho*nu,U)")
          + fvc::div( (iter()*iter().rho()*iter().myNu())*dev2(T(fvc::grad(U))),"div(rho*nu*dev2(T(gradU)))");
     } else {
      Info<< "***nonGNF fluid for phase "<< count <<endl;  
//       divTauMF += fvm::laplacian( iter()*iter().rho()*iter().etaS(), U, "laplacian(rho*nu,U)")
//          + fvc::div( (iter()*iter().rho()*iter().etaS())*dev2(T(fvc::grad(U))),"div(rho*nu*dev2(T(gradU)))");
...
Foam::multiphaseMixture::mu() const
{
    ...
    tmp<volScalarField> tmu = iter()*iter().rho()*iter().myNu();
    volScalarField& mu = tmu.ref();

    for (++iter; iter != phases_.end(); ++iter)
    {
        mu += iter()*iter().rho()*iter().myNu();
    }
